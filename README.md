...
Llama 2 7B LLM underperforms indicated by the 12.8% acceptance / passing rate in the pass@1 metric:

![underperformance](https://github.com/PengxuanW/portfolio/blob/main/images/lora_underperform.png)

By finetuning the Llama 2 LLM with LoRA, which is a relatively light-weight finetuning method with less requirement on dataset size, memory and time. In most cases, LoRA achieves similar performance as training a model from a newer, larger dataset. Our finetuned model only need 6 million parameters, which is only 0.09% of the 7 billion parameter in the original Llama 2 model. Illustration of how LoRA transforms the original parameters:
![lora_overview](https://github.com/PengxuanW/portfolio/blob/main/images/lora_explain.png)

Our model has a slightly better overall performance than Llama 2 7B, with larger acceptance rates in data structure related coding problems such as array, strings, matrix, and linked list.
![result_table](https://github.com/PengxuanW/portfolio/blob/main/images/lora_result.png)

An example piece of code generated by our model compared to Llama 2 7B:
![example](https://github.com/PengxuanW/portfolio/blob/main/images/lora_result_example.png)
